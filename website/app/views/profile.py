...
profile_blueprint = Blueprint('observe',__name__)
...
def profile():
    ...
    query = 'SELECT * FROM events WHERE id = ' + str(d['obs'])
    df = pd.read_sql_query(query, engine)
    temp = df.to_dict('records')
    columnNames = df.columns.values
    return render_template('galaxies.html', records=temp, colnames = columnNames)
